    ...
    const parsedData: Part[] = rows
      ...
      .map((row) => {
        ...
        headers.forEach((header, index) => {
          ...
          record[header.trim().toLowerCase()] = row[index] ? row[index].replace(/(^"|"$)/g, "").trim() : "";
        });

        return {
          id: record.id || crypto.randomUUID(),
          name: record.name || "Unnamed Part",
          category: record.category || "Accessories",
          image_url: record.image_url || "",
          description: record.description || "",
          price_visible: record.price_visible || "Contact us",
          enquiry_link: record.enquiry_link || "#contact",
        };
      });
